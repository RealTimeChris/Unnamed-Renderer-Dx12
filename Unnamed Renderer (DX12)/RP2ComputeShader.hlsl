...
void GetWSCamPathOrigin(in float2 NormalizedTSCoords, inout float3 WSCamPathOrigin) {
	WSCamPathOrigin = RootConstants.WSCameraLookFrom;
}
void GetWSCamPathDirection(in float2 NormalizedTSCoords, inout float3 WSCamPathDirection) {
	float3 W = normalize(RootConstants.WSCameraLookFrom - RootConstants.WSCameraLookAt);
	float3 U = normalize(cross(RootConstants.WSCameraUp, W));
	float3 V = cross(W, U);

	float3 HorizontalExtent = U * RootConstants.WSViewPortDimensions.x;
	float3 VerticalExtent = V * RootConstants.WSViewPortDimensions.y;

	float3 LowerLeftCorner = RootConstants.WSCameraLookFrom - (HorizontalExtent * 0.5f) - (VerticalExtent * 0.5f) - W;

	float3 TargetPoint = LowerLeftCorner + (NormalizedTSCoords.x * HorizontalExtent) + (NormalizedTSCoords.y * VerticalExtent);

	WSCamPathDirection = normalize(TargetPoint - RootConstants.WSCameraLookFrom);
}
...
void Schlick(in float CosThetaA, in float n1, in float n2, inout float RefractionProbability) {
	...
	R0 = R0 * R0;
	RefractionProbability = R0 + (1.0f - R0) * pow((1.0f - CosThetaA), 5.0f);
}
...
void UpdatePathFromDielectricIntersection(in uint2 GridThreadId, in IntersectionRecord HitRecord, inout Path CurrentPath) {
	...
	float SinThetaA = sqrt(saturate(1.0f - CosThetaA * CosThetaA));

	float RefractionRatio = n1 / n2;
	bool CannotRefract = (RefractionRatio * SinThetaA) > 1.0f;
	...
	if (CannotRefract || RefractionChance < RefractionProbability) {
		...
		float3 RefractedPerp = RefractionRatio * (HitRecord.WSIncomingPathDirection + CosThetaA * SurfaceNormal);
		float3 RefractedParallel = -sqrt(saturate(1.0f - dot(RefractedPerp, RefractedPerp))) * SurfaceNormal;

		CurrentPath.WSDirection = normalize(RefractedPerp + RefractedParallel);
		CurrentPath.WSOrigin = HitRecord.WSIntersectionPoint;
	}
}
